    ...
    switch (error.status) {
      case 400:
        ...
        if (toolName === 'create_campaign') {
          throw new McpError(
            ...
            `Campaign creation failed (400): ${error.message}. ` +
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `Bad request (400): ${error.message}. Please check your parameters and try again.`
        );
      
      case 401:
        throw new McpError(
          ErrorCode.InvalidRequest,
          `Authentication failed (401): ${error.message}. Please check your API key is valid and has not expired.`
        );
      ...
      case 403:
        ...
        if (toolName === 'verify_email') {
          throw new McpError(
            ErrorCode.InvalidRequest,
            `Email verification access forbidden (403): ${error.message}. ` +
        ...
        throw new McpError(
          ErrorCode.InvalidRequest,
          `Access forbidden (403): ${error.message}. You may not have permission for this operation or it may require a premium plan.`
        );
      ...
      case 404:
        ...
        if (toolName === 'list_leads') {
          throw new McpError(
            ErrorCode.InvalidRequest,
            `Leads not found (404): ${error.message}. ` +
            `This may indicate the endpoint or parameters are incorrect. Try creating a lead first with create_lead.`
          );
        }
        throw new McpError(
          ErrorCode.InvalidRequest,
          `Resource not found (404): ${error.message}. The requested resource may not exist or you may not have access to it.`
        );
      
      case 422:
        throw new McpError(
          ErrorCode.InvalidParams,
          `Validation error (422): ${error.message}${error.details ? `. Details: ${JSON.stringify(error.details)}` : ''}. ` +
      ...
      case 429:
        throw new McpError(
          ErrorCode.InvalidRequest,
          `Rate limit exceeded (429): ${error.message}. Please wait before retrying. ` +
