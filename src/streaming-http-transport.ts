...
export class StreamingHttpTransport {
  ...
  private setupRoutes(): void {
    ...
    // EMERGENCY FIX: Handle requests even without SSE session (SSE handshake is broken)
    this.app.post('/messages', async (req, res) => {
      ...
      // Source 1: Query parameter (fallback)
      if (apiKeyFromQuery) {
      ...
      // Source 3: Extract from sessionId if it looks like a base64 API key
      ...
      try {
        ...
        // Import the executeToolDirectly function from index.ts
        const { executeToolDirectly } = await import('./index.js');

        if (method === 'tools/call') {
          const { name, arguments: args } = params;
          console.error(`[HTTP] 🔧 Executing tool: ${name}`);
          ...
          const { TOOLS_DEFINITION } = await import('./index.js');
          return res.status(200).json({
            jsonrpc: '2.0',
